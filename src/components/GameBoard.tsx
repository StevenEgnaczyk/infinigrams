...
export const GameBoard = ({
  ...
  const shouldScale = Boolean(isVictory || showSolution || isAutoSolving)

  const calculateWaveCells = (
  ...
  const resolveNextState = (
    ...
    preferMarkEmpty: boolean
  ): boolean | 'x' => {
    if (dragState !== null) return dragState

    if (preferMarkEmpty) {
      return initialCell === 'x' ? false : 'x'
    }

    return initialCell === false ? true : initialCell === true ? 'x' : false
  }
  ...
  const handleCellInteraction = (
    ...
    colIndex: number,
    initialCell: boolean | 'x',
    preferMarkEmpty = false
  ) => {
    const nextState = resolveNextState(initialCell, preferMarkEmpty)
  ...
  const getCellClassName = (
    ...
    if ((isVictory || showSolution) && isCorrectCell) {
      return `w-8 h-8 wave-cell ${activeCell ? 'wave-active' : 'bg-yellow-600'}`
    }
            ...
            {game.userGrid.map((row, rowIndex) => (
              <div key={rowIndex} className="contents">
                ...
                {row.map((cell, colIndex) => {
                  const activeCell = activeWave.find(
                    (waveCell) =>
                      waveCell.row === rowIndex && waveCell.col === colIndex
                  )
                  const intensity = activeCell?.intensity ?? 0
                  const waveStyle =
                    (isVictory || showSolution) &&
                    game.solution[rowIndex][colIndex]
                      ? ({
                          '--wave-scale': String(1 + intensity * 0.4),
                          '--wave-opacity': String(intensity),
                        } as CSSProperties)
                      : undefined
